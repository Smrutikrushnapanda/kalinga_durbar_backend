...
export class Registration {
  ...
  @Column({ name: 'flight_details', type: 'varchar', length: 255, nullable: true })
  ...
  @ApiProperty({ example: ['jagannath', 'chilika'], description: 'Selected pre-tour IDs' })
  ...
  @Column({ name: 'status', type: 'varchar', length: 20, default: RegistrationStatus.PENDING })
  status!: string;

  // ── QR ────────────────────────────────────────────────────────────────────

  @ApiProperty({ example: 'KD-2026-ABCD1234', description: 'Unique QR code token' })
  @Column({ name: 'qr_token', type: 'varchar', length: 100, unique: true, nullable: true })
  qrToken?: string;

  // ── Relations ─────────────────────────────────────────────────────────────

  @OneToMany(() => AccompanyingPerson, (p) => p.registration, { cascade: true, eager: true })
